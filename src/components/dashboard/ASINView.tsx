...
export function ASINView() {
  ...
  const profitBeforeAds = costData.salePrice - costData.fees - costData.cogs;
              ...
              <div>
                ...
                {editingCosts ? (
                  <Input
                    ...
                    step="0.01"
                    value={costData.fees}
                    onChange={(e) => setCostData(prev => ({ ...prev, fees: parseFloat(e.target.value) || 0 }))}
                  />
                ) : (
                  <p className="text-lg font-semibold">{formatCurrency(costData.fees)}</p>
                )}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
